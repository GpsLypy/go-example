func mysqlReadDataByIntRange(task *Task) ([]RowData, error) {
	LogDebug("Do task: %v", task)

	dataSizeLimit := getConfig().DataSizeLimit
	// if task.IsSelfPri {
	// 	if dataSizeLimit < task.RowCountLimit {
	// 		dataSizeLimit = task.RowCountLimit
	// 	}
	// }
	...
	if task.IsSelfPri && task.CurrId == task.EndId {
		...
		//query += fmt.Sprintf(" ORDER BY `%s` LIMIT %d", task.PrimaryKey, dataSizeLimit)
...
func mysqlReadDataByStrRange(task *Task) ([]RowData, error) {
	...
	//字符串转义,此处不能重复转义
	//task.strCurrId = Stringescape(task.strCurrId)
	// create sql
