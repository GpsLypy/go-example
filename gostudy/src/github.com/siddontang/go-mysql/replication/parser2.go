...
func NewBinlogParser2() *BinlogParser2 {
	p := new(BinlogParser2)
...
func (p *BinlogParser2) Stop() {
	atomic.StoreUint32(&p.stopProcessing, 1)
}

func (p *BinlogParser2) Resume() {
	atomic.StoreUint32(&p.stopProcessing, 0)
}

func (p *BinlogParser2) Reset() {
	p.format = nil
}

func (p *BinlogParser2) ParseFile(name string, offset int64, onEvent OnEventFunc) error {
	f, err := os.Open(name)
	if err != nil {
...
func (p *BinlogParser2) parseFormatDescriptionEvent(r io.Reader, onEvent OnEventFunc) error {
	_, err := p.parseSingleEvent(r, onEvent)
	return err
}

// ParseSingleEvent parses single binlog event and passes the event to onEvent function.
func (p *BinlogParser2) ParseSingleEvent(r io.Reader, onEvent OnEventFunc) (bool, error) {
...
func (p *BinlogParser2) ParseReader(r io.Reader, onEvent OnEventFunc) error {
...
func (p *BinlogParser2) SetRawMode(mode bool) {
	p.rawMode = mode
}

func (p *BinlogParser2) SetParseTime(parseTime bool) {
	p.parseTime = parseTime
}

func (p *BinlogParser2) SetTimestampStringLocation(timestampStringLocation *time.Location) {
	p.timestampStringLocation = timestampStringLocation
}

func (p *BinlogParser2) SetUseDecimal(useDecimal bool) {
	p.useDecimal = useDecimal
}

func (p *BinlogParser2) SetIgnoreJSONDecodeError(ignoreJSONDecodeErr bool) {
	p.ignoreJSONDecodeErr = ignoreJSONDecodeErr
}

func (p *BinlogParser2) SetVerifyChecksum(verify bool) {
	p.verifyChecksum = verify
}

func (p *BinlogParser2) parseHeader(data []byte) (*EventHeader, error) {
	h := new(EventHeader)
	err := h.Decode(data)
	if err != nil {
		return nil, err
...
func (p *BinlogParser2) parseEvent(h *EventHeader, data []byte, rawData []byte) (Event, error) {
	var e Event
...
// an existing one.
func (p *BinlogParser2) Parse(data []byte) (*BinlogEvent, error) {
	rawData := data
...
func (p *BinlogParser2) verifyCrc32Checksum(rawData []byte) error {
	if !p.verifyChecksum {
		return nil
	}
...
func (p *BinlogParser2) newRowsEvent(h *EventHeader) *RowsEvent {
	e := &RowsEvent{}
