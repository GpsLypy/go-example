...
func (e *RowsEvent) Decode(data []byte) error {
	...
	defer func() {
		if r := recover(); r != nil {
			log.Fatalf("parse rows event panic %v, data %q, parsed rows %#v, table map %#v, table name %v\n%s",
				r, data, e, e.Table, string(e.Table.Table), Pstack())
		}
	}()
...
func (e *RowsEvent) decodeValue(data []byte, tp byte, meta uint16) (v interface{}, n int, err error) {
	...
	case MYSQL_TYPE_DATETIME:
		// If i64 is zero, directly return 0000-00-00 00:00:00
		// n = 8
...
func decodeDecimal(data []byte, precision int, decimals int, useDecimal bool) (interface{}, int, error) {
	...
	decimalStr := hack.String(res.Bytes())

	if useDecimal {
		f, err := decimal.NewFromString(decimalStr)
		return f, pos, err
	}
...
func decodeDatetime(data []byte) (interface{}, int, error) {
	...
	t := i64 % 1000000
	year := uint32(d / 10000)
	month := uint32((d % 10000) / 100)
	day := uint32(d % 100)
	hour := uint32(t / 10000)
	minute := uint32((t % 10000) / 100)
	second := uint32(t % 100)

	if 0 == year ||
		0 == month ||
		0 == day {
		// Can't parsed by golang Time package
		var mt mysqlTime
		mt.year = uint32(year)
		mt.month = uint32(month)
		mt.day = uint32(day)
		mt.hour = uint32(hour)
		mt.minute = uint32(minute)
		mt.second = uint32(second)
		mt.secondPart = uint64(0)
		mt.dec = int(0)
		return mt.String(), n, nil
	}
...
func decodeTime2(data []byte, dec uint16) (string, int, error) {
	...
	case 5, 6:
		...
		return TimeStringFromInt64TimePacked(tmp), n, nil
	default:
		intPart = int64(BFixedLengthInt(data[0:3])) - TIMEF_INT_OFS
		tmp = intPart << 24
	}
