...
func (gtid *MariadbGTID) forward(newer *MariadbGTID, skipLower bool) error {
...
func (s *MariadbGTIDSet) Sub(o GTIDSet) int64 {
	om, ok := o.(*MariadbGTIDSet)
	if !ok {
		return -1
	}
	if len(om.Sets) != len(s.Sets) {
		return -1
	}
	...
	for k, v := range s.Sets {
		ov, ok := om.Sets[k]
		if !ok {
			trx += int64(v.SequenceNumber)
		} else {
			sub := v.SequenceNumber - ov.SequenceNumber
			if sub > 0 {
				trx += int64(sub)
			}
		}
...
func (s *MariadbGTIDSet) Merge(GTIDStr string) error {
	gtid, err := ParseMariadbGTID(GTIDStr)
	if err != nil {
		return err
	}
...
func (s *MariadbGTIDSet) HasSID(sid string) bool {
	sidInt, err := strconv.ParseUint(sid, 10, 64)
	if nil != err {
		panic("Invalid domain id of mariadb gtid")
	}
	if nil == s.Sets {
		return false
	}
	_, ok := s.Sets[uint32(sidInt)]
	return ok
}
