...
func (s *MysqlGTIDSet) Sub(o GTIDSet) int64 {
	mo, ok := o.(*MysqlGTIDSet)
	if !ok {
		return -1
	}
	if len(s.Sets) != len(mo.Sets) {
		return -1
	}
	...
	for k, v := range s.Sets {
		ov, ok := mo.Sets[k]
		if !ok {
			trx += v.Intervals[len(v.Intervals)-1].Stop
		} else {
			sub := v.Intervals[len(v.Intervals)-1].Stop - ov.Intervals[len(ov.Intervals)-1].Stop
			if sub > 0 {
				trx += sub
			}
		}
...
func (s *MysqlGTIDSet) HasSID(sid string) bool {
	if nil == s.Sets {
		return false
	}
	_, ok := s.Sets[sid]
