...
//这句意思就是时间一到，ticker会通过管道发出一个信号给CPU，告诉CPU时间到了，该执行定时里的方法了，
//信号发出后，ticker会自动的重置定时的剩余时间，然后再进行下一轮的发送信号执行方法

// 但NewTimer()看源码会发现，它在sleep.go文件里定义的，也就是说NewTimer()相当于一个睡眠（延时执行）。

// 时间一到，timer会通过管道发出一个信号告诉CPU该执行定时里的代码了，然后这个管道就销毁了，
//除非使用 timer.Reset(time.Duration * 2) 来重新激活这根管道，让它重置定时的剩余时间，到下一轮定信号发出后，还要再次重置
